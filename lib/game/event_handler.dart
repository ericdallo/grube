...
class EventHandler {
  ...
  void handle(List<dynamic> message) async {
    if (message[0] == "chsk/handshake") {
      playerId = message.last[0];
      return;
    }

    message.forEach(handleSingleEvent);
  }

  void handleSingleEvent(dynamic event) {
    String eventName = event[0];
    var data = event[1];

    if (eventName == "game/player-added" && playerId == data['player']['id']) {
